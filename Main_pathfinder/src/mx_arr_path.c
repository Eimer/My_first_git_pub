...
static void free_mem(char **arr_dist, char **arr_from, char **arr_to) {
    int count = 0;
        
    while (arr_from[count]) {
        free(arr_from[count]);
        free(arr_to[count]);
        free(arr_dist[count]);
        count++;
...
static int **fill_arr_path(int **arr_path, char *str_from_file, int count, char **isl_no_repeat) {
    ...
    free_mem(arr_dist, arr_from, arr_to);
    return arr_path;
}
...
int **mx_arr_path(char *argv[]) {
    ...
    free(str_from_file);
    int test = 0;
    while (isl_no_repeat[test]) {
        free(isl_no_repeat[test]);
        test++;
    }
    free(isl_no_repeat);
    return arr_path;
